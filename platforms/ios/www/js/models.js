...
app.models.LocationCollection = Force.SObjectCollection.extend({
    ...
    config: function () {
        if (!app.offlineTracker.get("isOnline")) {
            if (this.key == null) {
                return {type:"cache", cacheQuery: {queryType:"smart", smartSql: "SELECT {location__c:_soup} FROM {location__c} WHERE {location__c:Location_ID__c} LIKE '%2015' ORDER BY {location__c:Location_ID__c} ASC"}};
            } else {
                return {type:"cache", cacheQuery: {queryType:"smart", smartSql: "SELECT {location__c:_soup} FROM {location__c} WHERE {location__c:Location_ID__c} LIKE '%" + this.key + "%' AND {location__c:Location_ID__c} LIKE '%2015' ORDER BY {location__c:Location_ID__c} ASC", pageSize: 25}};
            }
        } else {
            if (this.key == null) {
                return {type:"soql", query: "SELECT " + this.fieldlist.join(",") + " FROM Location__c WHERE Location_ID__c LIKE '%2015' ORDER BY Location_ID__c ASC"};
            } else {
                return {type:"soql", query: "SELECT " + this.fieldlist.join(",") + " FROM Location__c WHERE Location_ID__c LIKE '%" + this.key + "%' AND Location_ID__c LIKE '%2015' ORDER BY Location_ID__c ASC"};
            }
        }
    }
});
